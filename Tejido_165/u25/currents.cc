...
void human::currents(int x) {
	...
	ICaL_max = 1.018 * 4.0 * vffrt * (p->cass * exp(2.0 * vfrt) - 0.341 * cao)					
			/ (exp(2.0 * vfrt) - 1.0);
	ICaNa_max = 1.0 * vffrt * (0.75 * p->nass * exp(1.0 * vfrt) - 0.75 * nao)
			/ (exp(1.0 * vfrt) - 1.0);
	ICaK_max = 1.0 * vffrt * (0.75 * p->kss * exp(1.0 * vfrt) - 0.75 * ko)
			/ (exp(1.0 * vfrt) - 1.0);
	double fICaLp = (1.0 / (1.0 + 0.15 / CaMKa)); //CaMK affects

	ICaL =  ((1.0 - fICaLp) * PCa * ICaL_max * p->d
					* (f * (1.0 - p->nca) + p->jca * fca * p->nca)
					+ fICaLp * PCap * ICaL_max * p->d
							* (fp * (1.0 - p->nca) + p->jca * fcap * p->nca));

	ICaNa =  ((1.0 - fICaLp) * PCaNa * ICaNa_max * p->d
					* (f * (1.0 - p->nca) + p->jca * fca * p->nca)
					+ fICaLp * PCaNap * ICaNa_max * p->d
							* (fp * (1.0 - p->nca) + p->jca * fcap * p->nca));

	ICaK =  ((1.0 - fICaLp) * PCaK * ICaK_max * p->d
					* (f * (1.0 - p->nca) + p->jca * fca * p->nca)
					+ fICaLp * PCaKp * ICaK_max * p->d
							* (fp * (1.0 - p->nca) + p->jca * fcap * p->nca));

	IKr = GKr * xr * rkr * sqrt(ko / 5.4) * (p->v - EK);
	...
	double a1 = (k1p * pow(p->nai / Kpnai, 3.0))
			/ (pow(1.0 + p->nai / Kpnai, 3.0) + pow(1.0 + p->ki / Kpki, 2.0)
					- 1.0);
	double b1 = k1m * MgADP;
	double a2 = k2p;
	...
	double b4 = (k4m * pow(p->ki / Kpki, 2.0))
			/ (pow(1.0 + p->nai / Kpnai, 3.0) + pow(1.0 + p->ki / Kpki, 2.0)
					- 1.0);
	...
	ICab = PCab * 4.0 * vffrt * (p->cai * exp(2.0 * vfrt) - 0.341 * cao)
			/ (exp(2.0 * vfrt) - 1.0);
}
