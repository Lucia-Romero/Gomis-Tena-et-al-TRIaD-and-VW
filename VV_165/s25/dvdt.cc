...
void human::dvdt(int x)
{
        ...
        if (p->APDflag == 1 && p->v < (0.7 * p->vmax + 0.3 * p->vrest))
        {
                p->APD30 = t - p->tvdotmax;
                p->APDflag =  2;
        }
 
        if (p->APDflag == 2 && p->v < (0.5 * p->vmax + 0.5 * p->vrest))
        {
                p->APD50 = t - p->tvdotmax;
                p->APDflag =  3;
        }
        if (p->APDflag == 3 && p->v < (0.4 * p->vmax + 0.7 * p->vrest))
        {
                p->APD60 = t - p->tvdotmax;
                p->APDflag =  4;
        }
        if (p->APDflag == 4 && p->v < (0.1 * p->vmax + 0.9 * p->vrest))
        {
