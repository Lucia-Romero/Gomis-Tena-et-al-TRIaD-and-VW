...
public:
	...
	human();
	human(char argv1[], char argv2[], char argv3[], char argv4[], char argv5[], char argv6[], char argv7[], char argv8[], char argv9[], char argv10[],int argv11);
	humandata *p, *l, *r;
	...
	char fpth[250], fpath[250],ecg_path[250], apd_path[250], voltages_path[250], one_path[250],
			...
			qtint_path[250], TR_mx_path[250], mx_tail_path[250] ;       void    ECG(int x),
                        ecg_heading(),
                        ecg_save(int i),
                        apd_heading(),
                        apd_save(int x),
                        ...
                        gates(int x),
                        currents(int x),
                        stim(int x, int stim_here, int extra_stim_here),
                        voltage(int x),
                        dvdt(int x),
                        update(int x),
                        fluxbuf(int x),
                        concentrations(int x),
                        save_heading(int strandlength),
                        save(int x, int strandlength);
