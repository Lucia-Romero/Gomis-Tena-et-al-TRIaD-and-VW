...
void human::save_heading(int strandlength) {
	...
	fprintf(one,
			"%-07s\t%-07s\t%-07s\t%-07s\t%-07s\t%-07s\t%-07s\t%-07s\t%-07s\t%-07s\t%-07s\t%-07s\t%-07s\t%-07s\t%-07s\t%-07s\t%-07s\t%-07s\t%-07s\t%-07s\t%-07s\t%-07s\t%-07s\t%-07s\t%-07s\t%-07s\t%-07s\t%-07s\t%-07s\t%-07s\t%-07s\t%-07s\t%-07s\t%-07s\t%-07s\t%-07s\t%-07s\t%-07s\t%-07s\t%-07s\n",
			"t", "v", "nai", "nass", "ki", "kss", "cai", "cass", "cansr",
			"cajsr", "Jrel", "CaMKt", "Jup", "Jtr", "Jdiff", "JdiffNa",
			"JdiffK", "Jleak", "INa", "INaL", "Ito", "ICaL", "ICaNa", "ICaK",
			"IKr", "IKs", "IK1", "INaCa_i", "INaCa_ss", "INaK", "INab", "IKb",
			"IpCa", "ICab", "Ist", "dt", "APD90", "APD60", "APD50", "APD30");
	...
	if (strandlength >= 5) {
		...
		fprintf(two,
				...
				"t", "v", "nai", "nass", "ki", "kss", "cai", "cass", "cansr",
				"cajsr", "Jrel", "CaMKt", "Jup", "Jtr", "Jdiff", "JdiffNa",
				"JdiffK", "Jleak", "INa", "INaL", "Ito", "ICaL", "ICaNa",
				"ICaK", "IKr", "IKs", "IK1", "INaCa_i", "INaCa_ss", "INaK",
				"INab", "IKb", "IpCa", "ICab", "Ist", "dt", "APD90", "APD60",
				"APD50", "APD30");
		fclose(two);
...
void human::save(int x, int strandlength) {
	...
	if (x == 0 && save_flag == 0 && strandlength == 1) {
		...
		voltages = fopen(voltages_path, "a");
		fprintf(voltages, "%-07g\t", p->v);
		fclose(voltages);

	}
#endif //JULIO
	if( fmod(t,CL) > fmod(TR_max,CL) )
          p->mxtail = (p->mxtail < p->v) ? p->v : p->mxtail;
        else
          p->mxtail = p->vrest;
	...
	if (x == 0) {
		...
		fprintf(one,
				...
				t + dt, p->v, p->nai, p->nass, p->ki, p->kss, p->cai, p->cass,
				p->cansr, p->cajsr, Jrel, p->CaMKt, Jup, Jtr, Jdiff, JdiffNa,
