...
human::human(char argv1[], char argv2[], char argv3[], char argv4[], char argv5[], char argv6[],char argv7[], char argv8[], char argv9[], char argv10[],int argv11) 
{
	...
	strandlength = 165;
	stim_here = 0;
        extra_stim_here = (strandlength + 1)/2;
	ft = beats * CL;
	dt = 0.003;
	skip = 200;
	...
	sprintf(qtint_path,    "%s_QT_.txt", fpath);
        sprintf(TR_mx_path,    "%s_MX_.txt", fpath);
}
